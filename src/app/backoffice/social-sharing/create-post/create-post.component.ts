...
export class CreatePostComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.socialSharigService.updateSocialPlatformsStatus();
    ...
    this.networksForm = new FormArray([]);
    this.socialSharigService.publishersList.forEach( publisher => {
      if(publisher.provider == 'facebook' && publisher?.isPlatform) {
    ...
    if(this.socialSharingService.socialPlatformsStatus.facebook) {
      this.socialSharigService.getCurrentUsersFacebookGroups().then( (res: any) => {
        this.fbGroupsList = res.data;
  ...
  next(): void {
    ...
    if(this.current == 0) {
      ...
      if(this.selectedNetworks.length == 0) {
        this.showMessage('error', this.translate.instant("NoSocialNetworkSelected"));
        return;
      }
      let networkData = JSON.parse(JSON.stringify(this.networksForm.value));
      this.postData['networks'] = networkData;
      this.changeCurrent(1);
    } else {
      this.changeCurrent(1);
    }
      ...
      this.modal.confirm({
        ...
        nzOnOk: () =>
          new Promise((resolve, reject) => {
            this.socialSharigService.addNewPost(this.postData).then((data) => {
  ...
  onCheckboxChange(formGroupIndex, formControlName, selectedId, name, isChecked) {
    const fGroup = <FormGroup>this.networksForm.controls[formGroupIndex],
    fControl = fGroup.controls[formControlName],
    displayNameFormControl = fGroup.controls.displayName;
    
    if(isChecked) {
      let index = fControl.value.findIndex(x => x == selectedId)
      if (index == -1) {
        fControl.value.push(selectedId);
        displayNameFormControl.value.push(name);
      }
      index = this.selectedNetworks.findIndex(id => id == selectedId)
      if (index == -1) {
        this.selectedNetworks.push(selectedId);
      }
    } else {
      let index = fControl.value.findIndex(x => x == selectedId)
      if (index > -1) {
        fControl.value.splice(index, 1);
        let nameIndex = displayNameFormControl.value.findIndex(x => x == name)
        displayNameFormControl.value.splice(nameIndex, 1);
      }
    ...
    console.log(this.networksForm.value[0]);
  }

  isSelected(id) {
    let index = this.selectedNetworks.findIndex(x => x == id);
    if(index == -1) {
      return false;
    } 
    else 
