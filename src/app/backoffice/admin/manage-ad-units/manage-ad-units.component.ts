...
export class ManageAdUnitsComponent implements OnInit {
  ...
  private updateAdUnitForm(unit: AdUnit): void {
    this.adUnitForm.patchValue({
      title: unit.title,
  ...
  handleOk(): void {
    ...
    if (this.adUnitForm.valid) {
      ...
      if (this.editAdUnit) {
        this.adNetworkService.updateSiteAdUnit(this.site.publisher.id, this.editAdUnit.id, adUnitData).then(() => {
  ...
  public startEdit(adUnit: AdUnit): void {
    this.editAdUnit = adUnit;
    this.updateAdUnitForm(adUnit);
    this.isVisible = true;
  }
